...
function animateCount(node, target, format = (n) => n.toLocaleString('en-IN')) {
...
function skeleton() {
  return `<div class="card result"><div class="result-head">
      <div class="result-emoji skeleton" style="background:rgba(255,255,255,.05)"></div>
      <div class="result-title">
        <div class="skeleton" style="height:17px;width:210px"></div>
        <div class="skeleton" style="height:11px;width:280px;margin-top:8px"></div>
      </div>
    </div>
    ${[0, 1, 2].map(() => `<div class="shop">
      <div class="shop-rank skeleton" style="background:rgba(255,255,255,.05)"></div>
      <div class="shop-main">
        <div class="skeleton" style="height:14px;width:170px"></div>
        <div class="skeleton" style="height:10px;width:260px;margin-top:7px"></div>
      </div>
      <div class="skeleton" style="height:26px;width:64px"></div>
    </div>`).join('')}
  </div>`;
}
...
async function runSearch(term) {
  ...
  try {
    ...
    el('results').innerHTML = `<div class="card"><div class="empty"><span class="big">⚠️</span>${esc(err.message)}</div></div>`;
...
function renderProductCard(result, origin, idx) {
  ...
  const rows = shops.length
    ? shops.map((s) => {
        ...
        <li class="shop ${nearest ? 'is-nearest' : ''} ${s.inStock ? '' : 'is-out'}">
          ...
          <div class="shop-main">
            ...
            <div class="shop-meta">${esc(s.category)} · ${esc(s.address)} · ★ ${s.rating.toFixed(1)} · ${esc(s.opens_at)}–${esc(s.closes_at)}</div>
          </div>
          <div class="shop-dist">
            <div class="km">${esc(s.distanceLabel)}</div>
            <div class="eta">~${s.etaMin} min</div>
            <div class="prox"><i data-w="${Math.max(6, 100 - (s.distanceKm / maxKm) * 92)}%" style="width:0"></i></div>
          </div>
    ...
    : `<li><div class="empty">No nearby shop lists this product${state.inStockOnly ? ' in stock' : ''}.</div></li>`;

  return `
    <article class="card result" style="animation-delay:${idx * 70}ms">
      ...
      <div class="mapwrap" style="position:relative">
        ...
        <svg class="mapsvg" id="map-${idx}" viewBox="0 0 560 250" role="img"
             aria-label="Scatter map of shops stocking ${esc(p.name)}, positioned by real distance and direction from you"></svg>
        <div class="map-legend">
          <span><i style="background:#0ca30c"></i>In stock</span>
          <span><i style="background:#d03b3b"></i>Out of stock</span>
          <span><i style="background:#7c5cff"></i>You</span>
...
function drawMiniMap(svgId, result, origin) {
  ...
  parts.push(`<text class="ring-label" x="${cx - 4}" y="${cy - R - 6}">N</text>`);

  // you
  parts.push(`<circle class="you-halo" cx="${cx}" cy="${cy}" r="13"></circle>`);
  parts.push(`<circle class="you" cx="${cx}" cy="${cy}" r="5.5"></circle>`);
  parts.push(`<text class="dot-label" x="${cx + 12}" y="${cy + 4}" style="fill:#a78bfa">You</text>`);
  ...
  shops.forEach((s) => {
    ...
    parts.push(`<g class="shop-dot" data-name="${esc(s.store_name)}" data-dist="${esc(s.distanceLabel)}" data-qty="${s.quantity}" data-price="${s.price}">
      <circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${(r + 2.5).toFixed(1)}" fill="${fill}" opacity=".18"></circle>
      <circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${r.toFixed(1)}" fill="${fill}" stroke="#0a0c14" stroke-width="2"></circle>
      <text class="dot-num" x="${x.toFixed(1)}" y="${(y + 3).toFixed(1)}">${s.rank}</text>
    </g>`);

    if (s.rank === 1 && !narrow) {
      const anchor = x > cx ? 'start' : 'end';
      const off = x > cx ? r + 6 : -(r + 6);
      parts.push(`<text class="dot-label" x="${(x + off).toFixed(1)}" y="${(y - r - 4).toFixed(1)}" text-anchor="${anchor}">${esc(s.store_name)}</text>`);
    }
  });
  ...
  openModal({
    ...
    bodyHtml: `
      ...
      <div class="field-row">
        ...
        <div class="field" style="max-width:112px">
          <label for="rq-qty">Quantity</label>
          <input id="rq-qty" type="number" min="1" value="1">
        </div>
      </div>
      <div class="field">
        <label for="rq-note">Note for the shop <span style="font-weight:400;color:var(--ink-3)">(optional)</span></label>
...
async function renderMyRequests() {
  ...
  const rows = state.myRequests.map((r) => {
    ...
    const status = live
      ...
      : '<span class="badge badge-stock">Served</span>';
...
function init() {
  ...
  el('toggle-instock').addEventListener('click', (e) => {
    ...
    e.target.textContent = state.inStockOnly ? 'In-stock only ✓' : 'In-stock only';
